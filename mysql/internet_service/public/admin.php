<?php
ob_start(); // 🔥 MAGIC FIX: Output Buffering চালু করা হলো (Headers Error চিরতরে বন্ধ!)
session_start();
...
if ($action == 'add_expense' && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $db->prepare("INSERT INTO expenses (category, amount, description, expense_date) VALUES (?, ?, ?, ?)")
...
if ($action == 'assign_tech' && isset($_GET['uid']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($staff_id > 0) {
        $check = $db->prepare("SELECT ticket_id FROM tickets WHERE user_id = ? AND category = 'New Installation' AND status != 'resolved'");
        $check->execute([$uid]);

        if ($check->rowCount() == 0) {
            $msg = "Please go to the customer's address, setup the new internet connection, configure router and pull the fiber cable.";
            $db->prepare("INSERT INTO tickets (user_id, subject, category, message, status, assigned_to) VALUES (?, 'New Internet Connection Setup', 'New Installation', ?, 'open', ?)")->execute([$uid, $msg, $staff_id]);
        } else {
            $db->prepare("UPDATE tickets SET assigned_to = ? WHERE user_id = ? AND category = 'New Installation' AND status != 'resolved'")->execute([$staff_id, $uid]);
        }

        // 🔥 ম্যাজিক: স্টাফকে নোটিফিকেশন পাঠানো হচ্ছে
        $notif_msg = "🚨 New Job: You have been assigned for a New Connection Setup (Customer ID: #$uid). Please check your pending tasks.";
        $db->prepare("INSERT INTO notifications (user_id, message) VALUES (?, ?)")->execute([$staff_id, $notif_msg]);
    }
...
if ($action == 'mark_paid' && isset($_GET['id'])) {
    ...
    // 🔥 আপডেট: end_date কলামটিও সিলেক্ট করা হয়েছে সঠিক ক্যালকুলেশনের জন্য
    $inv = $db->prepare("SELECT i.user_id, i.amount, i.subscription_id, s.end_date, p.duration_days 
                         FROM invoices i 
    ...
    if ($invData) {
        ...
        // 🔥 স্মার্ট ক্যালকুলেশন: 
        // যদি মেয়াদ এখনো থাকে (expiry > today), তবে মেয়াদ শেষ হওয়ার দিন থেকে যোগ হবে। 
        // নাহলে আজ থেকে নতুন মেয়াদ শুরু হবে।
    ...
    // রিডাইরেক্ট লজিক (অ্যাডমিন বা স্টাফ পেজ অনুযায়ী)
    $redirect = (basename($_SERVER['PHP_SELF']) == 'admin.php') ? 'admin.php?page=billings' : 'staff_dashboard.php';
    header("Location: $redirect&msg=renewed");
    exit;
}
...
if ($action == 'add_staff' && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $db->prepare("INSERT INTO users (full_name, designation, email, password_hash, role, phone, address, status) VALUES (?, ?, ?, ?, 'staff', ?, ?, 'active')")->execute([$name, $designation, $email, $pass, $phone, $address]);
...
if ($page == 'dashboard') {
    ...
    $new_requests = $db->query("
        ...
        JOIN packages p ON s.package_id = p.package_id 
        WHERE s.start_date IS NULL OR s.status = 'pending' OR u.status = 'pending'
        ORDER BY u.user_id DESC
    ")->fetchAll(PDO::FETCH_ASSOC);

    $active_subs = $db->query("SELECT u.full_name, u.phone, p.name as package_name, s.end_date, DATEDIFF(s.end_date, CURDATE()) as days_left FROM subscriptions s JOIN users u ON s.user_id = u.user_id JOIN packages p ON s.package_id = p.package_id WHERE s.status = 'active' AND s.start_date IS NOT NULL ORDER BY days_left ASC")->fetchAll(PDO::FETCH_ASSOC);

    // 🔥 অ্যাডমিনের নোটিফিকেশন আনা হচ্ছে (শুধু লেটেস্ট ২টা ড্যাশবোর্ডে দেখাবে)
    $adminNotifs = $db->prepare("SELECT message FROM notifications WHERE user_id = ? ORDER BY sent_at DESC LIMIT 2");
...
?>
    <div class="mt-8">
            ...
            <div class="bg-gray-800 text-white rounded-xl shadow-md p-5 mb-8 border-l-4 border-red-500 relative">
                ...
                <ul class="space-y-2 text-sm text-gray-300">
                    <?php foreach ($admin_notifications as $n): ?>
                        <li class="bg-gray-700 p-2.5 rounded border border-gray-600"><i class="fa fa-angle-right mr-2 text-red-400"></i> <?php echo htmlspecialchars($n['message']); ?></li>
                    <?php endforeach; ?>
                </ul>
            </div>
        <?php endif; ?>
        ...
        <?php if (isset($_GET['msg']) && $_GET['msg'] == 'tech_assigned') echo "<p class='text-green-600 mb-4 font-bold p-3 bg-green-50 border border-green-200 rounded'><i class='fa fa-check-circle mr-2'></i> Technician successfully assigned!</p>"; ?>
                        ...
                        <tbody>
                            <?php foreach ($new_requests as $req): ?>
                                <tr class="border-b hover:bg-gray-50">
                                    <td class="py-3 px-4 font-bold text-gray-700"><?php echo htmlspecialchars($req['full_name']); ?> <br><span class="text-xs text-gray-500"><i class="fa fa-phone"></i> <?php echo htmlspecialchars($req['phone']); ?></span></td>
    ...
    <div class="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 h-fit">
            ...
            <form action="admin.php?action=add_expense" method="POST" class="space-y-4">
                ...
                <div><label class="block text-xs font-bold text-gray-600 mb-1">Amount (৳)</label><input type="number" name="amount" required class="w-full border px-3 py-2 rounded outline-none"></div>
...
?>
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mt-6">
        ...
        <?php if (isset($_GET['msg']) && $_GET['msg'] == 'tech_assigned') echo "<p class='text-green-600 mb-4 font-bold p-3 bg-green-50 border border-green-200 rounded'><i class='fa fa-check-circle mr-2'></i> Technician successfully assigned for line setup!</p>"; ?>
